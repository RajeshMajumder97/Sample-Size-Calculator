...
def main():
    ...
    p1 = st.sidebar.number_input("Proportion of disease in unexposed group (control) (%)",value=40.0,min_value=0.0,format="%.6g",max_value=99.9,help="Enter a percentage value (e.g., 40%)")
    R = st.sidebar.number_input("Anticipated Relative Risk (RR)", value=1.45,min_value=0.00001,format="%.6g",help= "plausible range > or < 1.")
    ...
    if(x== "Given"):
        designEffect= st.sidebar.number_input("Design Effect", value=1.0,min_value=1.0,max_value=2.0,format="%.6g",help= "Enter a decimal value (e.g., 1.5)")
        m=None
        ICC=None
    else:
        ...
        with tabs[1]:
            ...
            for i, conf in enumerate(conf_levels):
                for j, power_val in enumerate(powers):
                    ss = nSampleRR(p1=(p1/100),RR=R,Pw= power_val,Conf=conf,designEf=designEffect,dropOut=(drpt/100))

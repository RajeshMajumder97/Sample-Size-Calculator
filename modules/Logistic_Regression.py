...
def main():
    ...
    # Sidebar inputs
    P = st.sidebar.number_input("Overall Proportion of Disease", value=10.0, min_value=1.0,format="%.6g", max_value=99.99)
    OR = st.sidebar.number_input("Anticipated Odds Ratio (OR)", value=1.5, min_value=0.01,format="%.6g")
    R2 = st.sidebar.number_input("R-squared with other predictors (R²)", value=0.2, min_value=0.0, max_value=0.99,format="%.6g",help="Enter a decimal value (e.g., 0.05)")
    ...
    k = st.sidebar.number_input("Number of Predictors (k)", value=1, min_value=1, format="%.6g")

...
def main():
    ...
    sigma = st.sidebar.number_input("Standard Deviation (SD)",value=13.0,min_value=0.01,format="%.6g",help= "Enter a value >0")
    ...
    if(x== "Given"):
        ...
        ICC= st.sidebar.number_input("Intra-class Correlation (ICC) for clustering",min_value=0.0,format="%.6g",help="Enter a decimal value (e.g., 0.05)")
    ...
    with st.expander("Show the formula and the references"):
        ...
        st.latex(r"""
        n = \frac{(Z_{1-(\alpha/2)} + Z_{1-\beta})^2 \cdot \sigma^2}{\delta^2} \times \frac{DE}{1 - \text{Dropout\%}}
        """)

...
def main():
    ...
    rho = st.sidebar.number_input("Expected ICC (ρ)", value=0.8, min_value=0.01, max_value=0.99)
    ...
    if prec_type == 'Absolute Precision':
        d = st.sidebar.number_input("Absolute Precision (d)(%)", value=5.0, min_value=1.0, max_value=100.0)/100.0
    else:
        d_percent = st.sidebar.number_input("Relative Precision (%)", value=10.0, min_value=0.01, max_value=50.0)
        d = (d_percent / 100) * rho
        col1, col2 = st.columns(2)
        col1.metric("Relative Precision (%)", value=d_percent)
        col2.metric("Precision (d)", value=round(d, 4))

    k = st.sidebar.number_input("Number of Raters / Repeated Measures (k)", min_value=2, value=3)
    drpt = st.sidebar.number_input("Drop-Out (%)", value=0.0, min_value=0.0, max_value=50.0) / 100
    ...
    if x == "Given":
        ...
        m = st.sidebar.number_input("Number of clusters", min_value=2, value=4)
        ICC = st.sidebar.number_input("Intra-class Correlation (ICC) for clustering", value=0.05, min_value=0.0, max_value=1.0)
